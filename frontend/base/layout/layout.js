...
import { getAll } from "@/services/addonsService";
...
const Layout = (props) => {
  ...
  // check route apakah maintenance atau tidak
  useEffect(() => {
    const path = router.pathname;
    const dynamicRoute = /^\/pages\/.*$/;

    if (addon) {
      addon.map((ad) => {
        console.log(ad);
        if (
          dynamicRoute.test(path) &&
          ad.status === 2 &&
          path.startsWith(`/pages/${ad.name}`)
        ) {
          router.replace("/maintenance");
        }
      });
    }
  }, [router.pathname, addon, router]);
  ...
  return (
    <React.Fragment>
      <Head>
        ...
        <meta property="og:title" content="project"></meta>
